...
| Order | Skill | Purpose |
|-------|-------|---------|
| 1 | `04-dotnet-domain-entity-generator` | User entity and Role/Permission value objects |
| 2 | `12-dotnet-jwt-authentication` | JWT token generation, validation, refresh tokens |
| 3 | `13-dotnet-permission-authorization` | Permission-based access control |
| 4 | `06-dotnet-ef-core-configuration` | User and Role EF Core mappings |
| 5 | `07.2-dotnet-minimal-api-endpoints` | Auth endpoints (login, register, refresh) |

## Steps

### Step 1 — Create the User entity

> Using skill `04-dotnet-domain-entity-generator`, create a `User` aggregate root with Email (value object), PasswordHash, FirstName, LastName, and a collection of Roles. Include a `Create` factory method and an `UpdatePassword` method.

**Verify:** `User.cs` has private setters. `Email` is a value object with validation. Roles are a collection, not a string.

### Step 2 — Configure JWT authentication

> Using skill `12-dotnet-jwt-authentication`, add `JwtOptions`, `JwtService`, `JwtBearerOptionsSetup`, and `UserContext`. Configure refresh tokens stored in HttpOnly cookies. Register JWT authentication in `DependencyInjection`.
...
> Using skill `13-dotnet-permission-authorization`, add `HasPermissionAttribute`, `PermissionRequirement`, `PermissionAuthorizationHandler`, and `PermissionAuthorizationPolicyProvider`. Define permissions as constants.
...
> Using skill `06-dotnet-ef-core-configuration`, create configurations for `User`, `Role`, and the `UserRole` join table. Map the Email value object as an owned type. Seed default roles and permissions.
...
> Using skill `07.2-dotnet-minimal-api-endpoints`, create endpoints: `POST /auth/register`, `POST /auth/login`, `POST /auth/refresh`. Login returns an access token in the body and sets a refresh token cookie.
