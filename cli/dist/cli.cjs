...
function loadSkillsFrom(rootDir) {
  if (!(0, import_node_fs.existsSync)(rootDir)) return [];
  const entries = (0, import_node_fs.readdirSync)(rootDir).filter((name) => !name.startsWith(".")).map((name) => ({ name, full: (0, import_node_path.join)(rootDir, name) })).filter((e2) => (0, import_node_fs.statSync)(e2.full).isDirectory()).sort((a3, b3) => a3.name.localeCompare(b3.name));
  const skills = [];
  for (const entry of entries) {
    const skillMd = (0, import_node_path.join)(entry.full, "SKILL.md");
    if (!(0, import_node_fs.existsSync)(skillMd)) continue;
...
function copyDir(src, dst) {
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.dirname)(dst), { recursive: true });
  (0, import_node_fs2.cpSync)(src, dst, { recursive: true, force: true });
}
function symlinkOrCopyDir(src, dst) {
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.dirname)(dst), { recursive: true });
  if ((0, import_node_fs2.existsSync)(dst)) (0, import_node_fs2.rmSync)(dst, { recursive: true, force: true });
  if (process.platform === "win32") {
    copyDir(src, dst);
    return;
  }
  (0, import_node_fs2.symlinkSync)(src, dst, "dir");
}
function removeIfOurs(target, expectedRoot) {
  if (!(0, import_node_fs2.existsSync)(target) && !isBrokenSymlink(target)) return false;
  const root = realResolve(expectedRoot);
  if ((0, import_node_fs2.lstatSync)(target).isSymbolicLink()) {
    const resolved = safeRealpath(target);
    if (!resolved || !isUnder(resolved, root)) return false;
    (0, import_node_fs2.rmSync)(target, { force: true });
    return true;
  }
  const parent = realResolve((0, import_node_path2.dirname)(target));
  if (parent !== root && !isUnder(parent, root)) return false;
  (0, import_node_fs2.rmSync)(target, { recursive: true, force: true });
  return true;
}
function isBrokenSymlink(p) {
  try {
    return (0, import_node_fs2.lstatSync)(p).isSymbolicLink();
  } catch {
    return false;
  }
}
...
function isUnder(child, parent) {
  return child === parent || child.startsWith(parent + import_node_path2.sep);
}

// src/agents/claude-code.ts
var ClaudeCodeAgent = class {
  /** Override homedir for testing. */
  constructor(home = (0, import_node_os.homedir)()) {
    this.home = home;
  }
  home;
  id = "claude-code";
  ...
  detect(projectDir) {
    return (0, import_node_fs3.existsSync)((0, import_node_path3.join)(this.home, ".claude")) || (0, import_node_fs3.existsSync)((0, import_node_path3.join)(projectDir, ".claude"));
  }
  ...
  async uninstall(target) {
    const expectedRoot = (0, import_node_path3.join)(target, "..");
...
${skill.body}`;
  ...
  async uninstall(target) {
    removeIfOurs(target, (0, import_node_path5.dirname)(target));
  }
};
...
function detectAgents(projectDir) {
  return allAgents().filter((a3) => a3.detect(projectDir));
}
...
var ManifestStore = class {
  ...
  filter(query) {
    return this.read().entries.filter(
      (e2) => (query.skill === void 0 || e2.skill === query.skill) && (query.agent === void 0 || e2.agent === query.agent) && (query.scope === void 0 || e2.scope === query.scope)
    );
  }
  remove(predicate) {
    const m2 = this.read();
    m2.entries = m2.entries.filter((e2) => !predicate(e2));
    this.write(m2);
  }
  write(m2) {
...
// src/tui/prompts.ts
function startTui(title) {
  oe(title);
}
function endTui(message) {
  $e(message);
}
...
async function chooseAgents(all, detected) {
  const options2 = all.map((a3) => ({
    value: a3.id,
  ...
  return all.filter((a3) => selected.includes(a3.id));
}
...
async function chooseScope(supported) {
  ...
  if (lD(choice)) fail("Cancelled.");
  return choice;
}
async function chooseMethod() {
  if (process.platform === "win32") {
    f2.info("Detected Windows \u2014 defaulting to copy (symlinks require Developer Mode).");
    return "copy";
  }
  const choice = await ie({
...
async function confirmPlan(summary) {
  f2.message(summary);
  const yes = await se({ message: "Proceed?", initialValue: true });
  if (lD(yes)) fail("Cancelled.");
  return yes;
}
...
async function addCommand(cwd = process.cwd()) {
  ...
  if (detected.length > 0) {
    f2.info(`Detected: ${detected.map((a3) => a3.displayName).join(", ")}`);
  }
